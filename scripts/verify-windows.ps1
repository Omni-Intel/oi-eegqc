param([Parameter(Mandatory=$true)][string]$BaselineInstaller, [switch]$CheckUpdate)
$ErrorActionPreference = 'Stop'
$repo = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$expectedVersion = & (Join-Path $repo '.venv/Scripts/python.exe') -c 'from oi_eegqc import __version__; print(__version__)'
if ($LASTEXITCODE -ne 0) { throw 'Could not read current application version' }
...
if (-not $before.DisplayVersion) { throw 'Baseline installation has no version' }
...
if ($after.DisplayVersion -ne $expectedVersion) { throw 'Upgrade version mismatch' }
...
Write-Output "PASS: install $($before.DisplayVersion), upgrade $expectedVersion, startup, uninstall; user data retained. Logs: $testDir"
