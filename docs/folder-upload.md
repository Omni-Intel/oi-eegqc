...
2. 一个本地采集文件夹长期复用同一个 `uploadId`，对象固定写入本机收件箱 `D:\EEG_Data\inbox\<uploadId>\`。第二天重新上传时，同路径覆盖、新路径追加；本地删除的文件不会删除收件箱里的旧对象。只有「作为新采集上传」会申请新的 `uploadId`。
...
4. 普通文件通过预签名 PUT 传到内网收件主机 `172.16.1.249`。大于 5 GiB 的文件自动使用分片上传；取消后再次上传会查询已存在的分片并只补传缺失部分。
...
固定签名入口为 `http://172.16.1.249:8443`，只接受内网地址。客户端不保存对象存储密钥。本机收件服务把文件写入 `D:\EEG_Data\inbox\`。

文件字节直接 PUT 到同一台收件主机，不经过公网 TOS。客户端只接受主机 `172.16.1.249` / `127.0.0.1`，以及与清单匹配的 `eeg/inbox/` 对象键或本轮暂存对象键。公网 TOS 地址会被拒绝。
